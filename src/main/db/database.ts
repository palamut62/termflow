...
export function createAgentTeam(input: CreateAgentTeamInput): AgentTeamBundle {
  ...
  const ts = now()
  const teamId = nanoid()
  // If a custom/AI template payload is provided, build the role/task plan from it.
  if (input.template) {
    const tpl = input.template
    if (!tpl.members?.length) throw new Error('Template has no members defined')
    ...
    store.teamEvents.push({ id: nanoid(), teamId, type: 'team.created', message: `Team created from template: ${tpl.name}`, createdAt: ts })
    persist()
