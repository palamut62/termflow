...
function teamBundle(team: AgentTeam): AgentTeamBundle {
  return {
    // Legacy records predate runtimeType: default them to the workflow runtime.
    team: { ...team, runtimeType: team.runtimeType ?? 'workflow' },
...
export function createAgentTeam(input: CreateAgentTeamInput): AgentTeamBundle {
  ...
  const ts = now()
  const teamId = nanoid()
  const runtimeType = input.runtimeType === 'native' ? 'native' : 'workflow'
  ...
  // Native teams let Claude plan and spawn its own teammates: seed only a lead
  // placeholder; members and tasks are filled in by the native bridge sync.
  if (runtimeType === 'native') {
    const lead: TeamMember = { id: nanoid(), teamId, name: 'Team Lead', role: 'lead', provider: 'claude', status: 'idle' }
    store.agentTeams.push(team)
    store.teamMembers.push(lead)
    store.teamEvents.push({ id: nanoid(), teamId, type: 'team.created', message: 'Native agent team created.', createdAt: ts })
    persist()
...
export function updateAgentTeam(id: string, patch: Partial<Pick<AgentTeam, 'status' | 'name' | 'worktreePath' | 'worktreeBranch' | 'baseCommit' | 'appliedAt' | 'nativeTeamName'>>): AgentTeamBundle {
  const team = store.agentTeams.find((item) => item.id === id)
...
// Map an experimental native task/member status onto TermFlow's task states.
function mapNativeTaskStatus(status?: string): TeamTask['status'] {
  switch ((status || '').toLowerCase()) {
    case 'in_progress': case 'in-progress': case 'working': return 'working'
    case 'completed': case 'done': return 'completed'
    case 'blocked': return 'blocked'
    default: return 'ready'
  }
}
function mapNativeMemberStatus(status?: string): TeamMember['status'] {
  switch ((status || '').toLowerCase()) {
    case 'working': case 'in_progress': case 'busy': return 'working'
    case 'completed': case 'done': return 'completed'
...
// Reconcile the DB model of a native team with the read-only bridge snapshot.
// Members are matched by name, tasks by title; unknown ones are inserted.
export function syncNativeTeamState(teamId: string, state: NativeTeamState): void {
  const team = store.agentTeams.find((item) => item.id === teamId)
  if (!team) return
  let changed = false
  for (const m of state.members) {
    const name = (m.name || '').trim()
    if (!name) continue
    const existing = store.teamMembers.find((x) => x.teamId === teamId && x.name.toLowerCase() === name.toLowerCase())
    const status = mapNativeMemberStatus(m.status)
    if (existing) {
      if (existing.status !== status) { existing.status = status; changed = true }
    } else {
      store.teamMembers.push({ id: nanoid(), teamId, name, role: m.lead ? 'lead' : 'developer', provider: 'claude', status })
      changed = true
    }
  }
