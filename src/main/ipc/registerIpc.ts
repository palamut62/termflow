...
import type { PtyBackend, PtyBackendStatus, PtyController } from '../pty/backend'
...
export function registerIpc(getWindow: () => BrowserWindow | null): PtyController {
  ...
  const backendReady: Promise<void> = (async () => {
  ...
  ipcMain.handle(IPC.PTY_CREATE, async (_e, id: string, input: CreateTerminalInput) => {
    ...
    return backend.create(id, { ...input, env: { ...(await workspaceEnv(input.workspaceId, input.cleanProviderEnv)), ...(input.env || {}) } })
  })
  ...
  ipcMain.handle(IPC.PTY_DAEMON_SHUTDOWN, async (): Promise<PtyBackendStatus> => {
    await backendReady
    if (daemon) {
