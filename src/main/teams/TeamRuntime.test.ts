...
const spawned: Array<EventEmitter & { stdout: EventEmitter; stderr: EventEmitter; kill: () => void }> = []
...
vi.mock('fs', () => ({ existsSync: () => false, mkdirSync: () => undefined }))
...
describe('agent team runtime adapters', () => {
  it('builds a structured Claude command without shell interpolation', () => {
    ...
    // Command resolves from PATH (may be an absolute claude.cmd/.exe or fall back to 'claude').
    expect(/claude/i.test(spec.command) || spec.args.some((a) => /claude/i.test(a))).toBe(true)
...
function makeBundle(over: Partial<AgentTeamBundle['team']> = {}, tasks: AgentTeamBundle['tasks'] = []): TeamStub {
  return {
    team: { id: 't1', workspaceId: 'w1', name: 'Team', objective: 'Do it', status: 'draft', permissionPolicy: 'balanced', runtimeType: 'workflow', createdAt: '', updatedAt: '', ...over },
