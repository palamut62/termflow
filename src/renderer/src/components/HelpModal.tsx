...
const topics: HelpTopic[] = [
  ...
  { id: 'panes', category: 'Windows', title: 'Split a window into panes', summary: 'Panes divide the selected window, exactly like tmux panes.', steps: ['Press Ctrl+Shift+D to split vertically or Ctrl+Shift+E to split horizontally.', 'Or use Split pane from the ⋯ menu in the window header.', 'Drag the splitter between two panes to change their ratio.', 'Use the pane tabs in the header to switch or close panes.', 'With the tmux prefix: prefix % splits vertically, prefix " splits horizontally.', 'prefix h/j/k/l or the arrow keys move to the pane in that direction; prefix o cycles to the next pane.', 'prefix z zooms the active pane to the full window; press it again to unzoom.', 'prefix x closes the active pane (the last pane asks how to close the window).'], notes: ['Pane ratios are saved with the workspace.', 'Zoom is not persisted, and any split or pane close drops it — exactly like tmux.'] },
  { id: 'providers', category: 'AI providers', title: 'Configure DeepSeek, Ollama, or another provider', summary: 'Provider profiles connect a terminal command to model and API endpoint environment variables.', steps: ['Open the New Terminal menu.', 'Choose Configure providers.', 'Enter the CLI command, model, base URL, and variable names.', 'Save the profile.', 'Add the API key separately in Settings > Developer > Workspace Environment.', 'Reopen the New Terminal menu and select the provider to launch it.'], notes: ['API keys are never stored inside provider profiles.', 'DeepSeek and Ollama starter profiles are included and may be edited.'] },
  ...
  { id: 'agent-config', category: 'Profiles', title: 'Claude config editor', summary: 'Edit Claude configuration files through a form instead of hand-editing JSON.', steps: ['Click the SlidersHorizontal icon in the toolbar.', 'Edit ~/.claude/settings.json for model, env, and co-authored-by.', 'Edit ~/.claude.json for theme, notification channel, auto-update, and verbose.', 'Save your changes.'], notes: ['A .termflow-bak backup is written before each save.', 'Restart your Claude terminals so the new configuration takes effect.'] },
  { id: 'broadcast', category: 'Workflows', title: 'Broadcast keyboard input', summary: 'Send the same keystrokes to multiple terminals at once.', steps: ['Use Add to broadcast group in the ⋯ menu of each window.', 'Enable Broadcast in the main header.', 'Type in the active terminal.', 'Disable Broadcast before returning to single-terminal input.'], notes: ['Commands are executed independently by every terminal in the group.'] },
  ...
  { id: 'plugins', category: 'Developer tools', title: 'Plugins', summary: 'Install declarative command plugins.', steps: ['Choose the puzzle icon in the header.', 'Install a versioned plugin JSON manifest.', 'Review every exposed command before running it.'], notes: ['Plugins cannot inject renderer JavaScript.'] },
  ...
  { id: 'shortcuts', category: 'Reference', title: 'Keyboard shortcuts', summary: 'Reach common actions without leaving the terminal.', steps: ['Ctrl+K — command palette', 'Ctrl+F — search inside the active terminal', 'Ctrl+Alt+T — new window', 'Ctrl+Tab / Ctrl+Shift+Tab — next / previous window', 'Ctrl+Shift+D — split the window into panes vertically', 'Ctrl+Shift+E — split the window into panes horizontally', 'Ctrl+Shift+B — toggle broadcast'] },
  { id: 'prefix', category: 'Reference', title: 'tmux prefix key', summary: 'Press the prefix (Ctrl+A by default), then a command key — exactly like tmux.', steps: ['prefix % — split the pane vertically', 'prefix " — split the pane horizontally', 'prefix h / j / k / l or arrow keys — move to the pane in that direction', 'prefix o — next pane', 'prefix x — close the active pane (last pane: close the window)', 'prefix z — zoom the active pane / unzoom', 'prefix c — new window', 'prefix n / p — next / previous window', 'prefix 0-9 — jump to a window by tab position', 'prefix , — rename the active window', 'prefix d — detach the active window', 'prefix [ — copy mode (not implemented yet)', 'prefix ? — this shortcut list'], notes: ['The prefix key is configurable in Settings > Terminal (Ctrl+A or Ctrl+B).', 'Pressing the prefix twice (e.g. Ctrl+A Ctrl+A) sends the key itself to the terminal, so readline’s Ctrl+A "beginning of line" still works.', 'A PREFIX badge appears in the status bar while the prefix waits; it clears after 2 seconds or on an unknown key, which is then delivered to the terminal.', 'The old shortcuts (Ctrl+Shift+D/E, Ctrl+Tab, ...) keep working alongside the prefix.'] },
